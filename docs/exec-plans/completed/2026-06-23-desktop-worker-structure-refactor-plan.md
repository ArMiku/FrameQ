...
- [x] 2026-07-10: P2 worker pipeline/media structure follow-up completed. `run_worker_pipeline` now reads as high-level orchestration after extracting media download/selection, video validation, audio preparation, subtitle/ASR stages, and optional AI finalization; `media.py` fallback dispatch is centralized through `DownloadStrategy` / `FALLBACK_DOWNLOAD_STRATEGIES` while preserving platform order and wire shapes.
...
2026-07-10 P2 worker pipeline/media follow-up: The worker pipeline/media structural round is closed. `run_worker_pipeline` now composes focused stage functions for context preparation, media download/selection, validation, audio preparation, subtitle/ASR, and optional AI finalization. `worker/frameq_worker/media.py` now centralizes fallback dispatch with `DownloadStrategy` and `FALLBACK_DOWNLOAD_STRATEGIES`, preserving Douyin -> Xiaohongshu -> Bilibili order and keeping YouTube on the existing failure-classification path. Deferred risk remains for real platform page/API drift, future `download_strategies.py` extraction only when platform strategy weight grows, and Python 3.13 `pydub/audioop` compatibility.

...
- [x] Surface the app-local `.env` path in desktop settings and provide a locate-file action.
- [x] Create a commented app-local `.env` template when settings are opened or saved and the file is missing.
...
- The settings panel is the user-facing discovery point for the app-local `.env`; repository-root `.env` remains a development artifact, not runtime config.

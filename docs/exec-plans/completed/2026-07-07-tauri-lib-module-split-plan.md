...
- [x] 2026-07-07: Extracted desktop diagnostics, log redaction, and worker result log summarization into `app/src-tauri/src/diagnostics.rs`. `lib.rs` dropped from 2545 to 2303 lines and `diagnostics.rs` now owns 276 lines. Validation: `cargo test --manifest-path app\src-tauri\Cargo.toml desktop_log_path -- --nocapture`, `cargo test --manifest-path app\src-tauri\Cargo.toml desktop_log_redacts -- --nocapture`, `cargo test --manifest-path app\src-tauri\Cargo.toml worker_result_log_summary -- --nocapture`, and `cargo test --manifest-path app\src-tauri\Cargo.toml diagnostic_text -- --nocapture` each passed with 1 focused test; `cargo test --manifest-path app\src-tauri\Cargo.toml` passed with 57 tests.
- [x] 2026-07-07: Extracted worker command construction, worker process state, subprocess spawning, stdout parsing, blocking execution, cancellation process-tree termination, and worker-command tests into `app/src-tauri/src/worker_command.rs`. `lib.rs` dropped from 2303 to 1526 lines and `worker_command.rs` now owns 615 lines. Validation: `cargo test --manifest-path app\src-tauri\Cargo.toml worker_process_state -- --nocapture` passed with 3 focused tests; `cargo test --manifest-path app\src-tauri\Cargo.toml worker_command_spec -- --nocapture` passed with 3 focused tests; `cargo test --manifest-path app\src-tauri\Cargo.toml blocking_worker_command -- --nocapture` passed with 1 focused test; `cargo test --manifest-path app\src-tauri\Cargo.toml parse_worker_stdout -- --nocapture` passed with 1 focused test; `cargo test --manifest-path app\src-tauri\Cargo.toml parse_worker_output -- --nocapture` passed with 2 focused tests; `cargo test --manifest-path app\src-tauri\Cargo.toml windows_worker_subprocesses -- --nocapture` passed with 1 focused test; `cargo test --manifest-path app\src-tauri\Cargo.toml` passed with 57 tests.
- [x] 2026-07-07: Extracted ASR model state, first-run status, ASR model download orchestration, model availability checks, ASR download command construction, and ASR-focused tests into `app/src-tauri/src/asr_model.rs`. `lib.rs` dropped from 1526 to 1069 lines and `asr_model.rs` now owns 491 lines. Validation: `cargo test --manifest-path app\src-tauri\Cargo.toml asr_model_availability -- --nocapture` passed with 3 focused tests; `cargo test --manifest-path app\src-tauri\Cargo.toml model_download_command_spec -- --nocapture` passed with 1 focused test; `cargo test --manifest-path app\src-tauri\Cargo.toml release_supported_asr_models -- --nocapture` passed with 1 focused test; `cargo test --manifest-path app\src-tauri\Cargo.toml` passed with 57 tests; `python scripts/validate_agents_docs.py --level WARN` completed with 0 errors and 0 warnings; `git diff --check` passed.
- [x] 2026-07-07: Extracted process/retry video command orchestration, cache reuse, worker request/result types, cancellation result, ASR model override application, and video-processing tests into `app/src-tauri/src/video_processing.rs`. `lib.rs` dropped from 1069 to 377 lines and `video_processing.rs` now owns 726 lines. Validation: `cargo test --manifest-path app\src-tauri\Cargo.toml cached_process_result -- --nocapture` passed with 3 focused tests; `cargo test --manifest-path app\src-tauri\Cargo.toml retry_insights_request -- --nocapture` passed with 1 focused test; `cargo test --manifest-path app\src-tauri\Cargo.toml apply_configured_asr_model -- --nocapture` passed with 1 focused test; `cargo test --manifest-path app\src-tauri\Cargo.toml` passed with 57 tests; `python scripts/validate_agents_docs.py --level WARN` completed with 0 errors and 0 warnings; `git diff --check` passed.
- [x] 2026-07-07: Extracted deep-link activation helpers and focused test into `app/src-tauri/src/deep_link.rs`, leaving `tauri_plugin_single_instance::init` setup in `lib.rs` and calling the extracted helper from the callback. `lib.rs` dropped from 377 to 306 lines and `deep_link.rs` now owns 80 lines. Validation: `cargo test --manifest-path app\src-tauri\Cargo.toml deep_link_activation -- --nocapture` passed with 1 focused test; `cargo test --manifest-path app\src-tauri\Cargo.toml` passed with 57 tests; `python scripts/validate_agents_docs.py --level WARN` completed with 0 errors and 0 warnings; `git diff --check` passed.
- [x] 2026-07-07: Completed final composition-root cleanup and closeout verification. Final module sizes are `lib.rs` 345 lines, `runtime.rs` 156 lines, `diagnostics.rs` 276 lines, `worker_command.rs` 689 lines, `asr_model.rs` 539 lines, `video_processing.rs` 776 lines, and `deep_link.rs` 96 lines. Validation: `cargo test --manifest-path app\src-tauri\Cargo.toml desktop_worker_contract -- --nocapture` passed with 1 focused test; `cargo test --manifest-path app\src-tauri\Cargo.toml` passed with 57 tests; `npm --prefix app test` passed with 23 files and 141 tests; `python scripts/validate_agents_docs.py --level WARN` completed with 0 errors and 0 warnings; `git diff --check` passed.
...
Evidence: ASR model download still constructs `WorkerCommandSpec` before the ASR module extraction, so `WorkerCommandSpec` fields and its test-only `env_map()` helper need `pub(crate)` visibility during the intermediate split. This keeps the ASR model download test in `lib.rs` while worker-specific tests live in `worker_command.rs`.

Evidence: During ASR extraction, `runtime.rs` still imported the root-level `asr_model_dir()` helper only to create the app-local `models` directory. Keeping that import after moving ASR into `asr_model.rs` would create a reverse dependency from runtime to ASR. The runtime helper now creates `paths.user_data_dir.join("models")` directly, while ASR owns model availability and marker validation.
...
Implemented. `app/src-tauri/src/lib.rs` now acts as the Tauri app composition root: module declarations, compatibility re-exports for existing modules, `greet`, plugin/state setup, command registration, and cross-module bootstrap/contract tests. Runtime path handling, diagnostics/redaction, worker command construction and subprocess handling, ASR model state/download commands, process/retry orchestration, cache reuse, and deep-link activation now live in focused modules. Validation completed with `cargo test --manifest-path app\src-tauri\Cargo.toml desktop_worker_contract -- --nocapture`, `cargo test --manifest-path app\src-tauri\Cargo.toml`, `npm --prefix app test`, `python scripts/validate_agents_docs.py --level WARN`, and `git diff --check`. Residual risk: root-level compatibility re-exports remain intentionally to avoid broad import churn; a future cleanup may move existing modules to direct imports once there is another nearby Rust maintenance pass.

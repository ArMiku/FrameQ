...
| GitHub Actions Node.js 20 action runtime warning | The v0.2.16 Desktop Release succeeded, but GitHub warned that checkout v4, setup-node v4, and setup-uv v6 target deprecated Node.js 20 and are currently forced onto Node.js 24. A future hosted-runner compatibility removal could turn this warning into a workflow failure. | Desktop Release run `29190905338`; `.github/workflows/desktop-release.yml` | Upgrade each affected action to a reviewed Node.js 24-capable major version, update focused workflow contract tests, and obtain green Windows/Intel/Apple Silicon release evidence before the next release that GitHub announces as enforcement-bound. |

...
| Permanent task deletion lacks macOS-native evidence | Windows real-filesystem, locked-file, symlink/junction, and partial-failure fixtures pass locally, while Unix link cases are conditionally compiled but were not executed on a macOS host in this session. | `docs/exec-plans/active/2026-07-12-history-task-permanent-deletion-plan.md`; `app/src-tauri/src/history_deletion.rs` | Run the full Rust suite and temporary-root deletion acceptance on supported macOS release hosts; close the active plan only after both architectures required for release have green evidence. |

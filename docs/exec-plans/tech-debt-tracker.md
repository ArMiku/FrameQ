...
| Raw source URL remains visible to local process inspection | The transient source URL is carried in a child-process JSON argument for cache preflight and full worker processing. FrameQ redacts application logs and never persists it, but a same-user process inspector or crash dump can observe it while that child is alive. | `app/src-tauri/src/worker_command.rs`; `docs/product-specs/2026-07-10-source-url-privacy-boundary.md` | Replace command-line JSON transport with a protected stdin pipe or equivalent inherited handle, and verify packaged Windows/macOS cancellation and diagnostics behavior. |
| Unix process-group cancellation needs native-host confirmation | Rust includes a `cfg(unix)` parent-plus-child fixture and the runtime uses an isolated PGID with TERM→KILL escalation, but this Windows workspace cannot execute that fixture or prove macOS/Linux `kill` behavior. | `app/src-tauri/src/worker_command.rs`; `docs/product-specs/2026-07-10-desktop-process-supervision-cancellation.md` | Run `cargo test --manifest-path app/src-tauri/Cargo.toml` on supported macOS and Linux release hosts; retain the result in the release evidence before claiming Unix cancellation verification. |

...
| Unix process-group cancellation needs native-host confirmation | Rust includes a `cfg(unix)` parent-plus-child fixture and the runtime uses an isolated PGID with TERM→KILL escalation. A read-only Ubuntu/macOS GitHub Actions workflow and static contract test are prepared, but this Windows workspace cannot execute the fixture and the workflow has not been pushed or run on hosted Unix machines. | `app/src-tauri/src/worker_command.rs`; `.github/workflows/unix-process-supervisor.yml`; `scripts/tests/unix-process-supervisor-workflow.test.mjs`; `docs/product-specs/2026-07-10-desktop-process-supervision-cancellation.md` | After explicit authorization, push the workflow and retain one run where both `ubuntu-latest` and `macos-latest` complete `cargo test --manifest-path app/src-tauri/Cargo.toml` successfully. Close this item only after recording the run URL, run ID, commit SHA, and both job results. |

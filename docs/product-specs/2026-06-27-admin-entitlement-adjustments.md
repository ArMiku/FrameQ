...
FrameQ is still stabilizing. When a user loses time or insight-generation quota because of a product bug, platform download instability, release regression, or support incident, the administrator needs a controlled way to compensate that user without issuing a new activation code. The compensation path should extend the user's entitlement expiry and add insight-generation uses while preserving the existing account, activation, and quota model.

## Goals

- Let the configured administrator manually extend a user's entitlement expiry from Admin Web.
- Let the configured administrator manually add insight-generation quota to a user after a support incident.
...
- The administrator can add a positive number of insight-generation uses. The default operation should increase `llmQuotaLimit` by the added amount and preserve `llmQuotaUsed`, so already consumed uses remain auditable.
...
- A compensated user sees the updated expiry and remaining insight-generation uses the next time the desktop client refreshes account status.
...
- Admin can add positive insight-generation uses without changing `llmQuotaUsed`.

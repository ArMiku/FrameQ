...
- 用户在确认页点击 `确认` 后，一次 AI整理固定消耗 1 次额度。
- 这 1 次额度覆盖本次 AI整理里的 `要点总结`、Mermaid mindmap 和 `启发话题点` 三项产物，不按产物分别扣除。
- 如果 AI整理开始后失败、部分失败或 LLM 调用失败，本次额度仍然扣除，不自动返还。
- `返回修改`、`取消`、只进入向导或只进入确认页不消耗额度。
- 确认页点击 `取消` 会废弃本次临时 6 步选择，不更新 `defaultGenerationPreferences`。
- `换个方向` 会重新打开本次 6 步偏好并进入确认页；用户再次点击 `确认` 后，会作为一次新的 AI整理再次消耗 1 次额度。
...
- 一次确认后的 AI整理消耗 1 次额度，覆盖要点总结、Mermaid mindmap 和启发话题点；失败或部分失败也扣除，`换个方向` 后再次确认会再次扣除。

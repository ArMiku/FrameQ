...
| `AI 整理中` | 可先展示文字稿，要点总结和话题点区域显示生成中 |
| `文字稿完成` | 主界面展示结果总览卡片，不直接铺满全文 |
| `部分完成` | 保留文字稿和已成功生成的 AI 产物，要点总结/话题点卡片展示失败态和重试入口 |
...
- 主按钮文案固定为 `确认`。
- 处理和完成态不再显示 URL 输入区域。
- 完成态主界面展示 `视频文件`、`音频文件`、`完整文字稿`、`要点总结` 和 `启发话题点` 5 个产物入口；视频和音频入口只定位本地文件，不打开详情浮窗。
...
- 完成态使用紧凑 document/result tiles：展示 `视频文件`、`音频文件`、`完整文字稿`、`要点总结` 和 `启发话题点` 5 个结果入口，不直接铺满全文，也不把入口卡拉成大面积空白卡；总结和话题点未生成时显示待生成状态和确认入口。
...
- `启发话题点` 个性化入口遵循 `docs/product-specs/2026-07-06-personalized-insight-preferences.md`。
...
- AI整理确认页必须展示本地灵感档案摘要、本次偏好摘要、额度消耗和云端 LLM 数据提示；额度文案应说明本次确认会消耗 1 次 AI整理额度，覆盖要点总结、Mermaid mindmap 和启发话题点，失败或部分失败也扣除，`换个方向` 后再次确认会再次扣除。
- 生成后的话题点详情应展示 `话题点`、`匹配理由`、`启发问题` 和 `适合用途`，而不是只展示问题列表。

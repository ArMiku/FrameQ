"""Tests for ``build_draft_from_inspiration_prompt`` (Task 1.1 / 1.4).

Covers the Prompt Strategy field mapping from design.md (D2):
- topic → central topic / title direction
- followUpQuestions → section skeleton / sub-arguments
- suitableUse → form & platform wording (drives skill loading, D4)
...
def test_prompt_includes_source_chunk_id_as_annotation_only() -> None:
    ...
    text must never enter the prompt (D2). The function never receives chunk
...
def test_suitable_use_values_produce_form_wording() -> None:
    """suitableUse drives form wording across the v1-supported platforms (D4)."""
    for use, marker in [
        ("抖音", "抖音"),
        ("小红书", "小红书"),
...
def test_unknown_suitable_use_falls_back_without_fabrication() -> None:
    """Per D4: non-v1 suitableUse falls back to LLM general capability — the
...
def test_prompt_without_snapshot_does_not_fabricate_persona() -> None:
    """D2 hard rule: missing snapshot → no personalization; NEVER fabricate a

"""Task 2.3 / 2.4 / 2.6: ``retry_insights_once`` draft branch.
...
branch of ``retry_insights_once``) plus the seed-validation contract (D8: invalid
...
def test_draft_branch_success_writes_draft_md_and_populates_process_result(
    ...
    # Preference snapshot read from disk (D2 / A1) — not sent over the wire.
    assert isinstance(captured["snapshot"], PreferenceSnapshot)
    # Summary read from disk (D2 optional grounding).
    ...
    # ProcessResult carries draft text + status (D7 / 2.1).
...
# --------------------------------------------------------------------------- #
# 2.6 seed invalid (D8: MUST NOT consume checkout / LLM call)
# --------------------------------------------------------------------------- #
...
def test_draft_branch_seed_not_in_insights_json_fails_without_running_draft(
    ...
    assert result["error"]["stage"] == "draft_generating"
    # Hard spec: invalid seed MUST NOT consume an LLM call / checkout.
...
# --------------------------------------------------------------------------- #
# 2.6 preference snapshot missing → None degrade (D2 graceful)
# --------------------------------------------------------------------------- #

...
def test_resolve_server_llm_flattens_five_fields_and_preserves_max_tokens() -> None:
    ...
    env = {
        ...
        "FRAMEQ_LLM_MAX_TOKENS": "4096",  # 非 server 托管 → 原样保留（design D6）
    }
    ...
    # 一篇只 checkout 一次（design D1）；鉴权头复用桌面端 session token。
...
def test_resolve_server_anysearch_with_key_overrides_local() -> None:
    ...
    # body 无 request_id（anysearch 无配额、无可幂等消费对象，design D4）。
...
def test_resolve_propagates_anysearch_checkout_error_as_neutral_runtime_error() -> None:
    # anysearch checkout 不复用 insight 码 → 中性 RuntimeError（design D10）。
    boom = HTTPError(

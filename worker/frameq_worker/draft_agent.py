...
class DraftSink:
    """成稿交付接收槽：``submit_draft`` 工具的唯一写入目标（design D3）。
...
def _build_submit_draft_tool(sink: DraftSink) -> FunctionTool:
    """构造 ``submit_draft`` 工具：把稿子正文写入共享接收槽（成稿交付信道，design D1/D2）。
...
class _CompositeHooks(BaseAgentRunHooks):
    ...
    Skills 经渐进式披露交付平台文体（design D1），inner hook 是 ``PlanningHook``
    （planning 开）或 no-op ``BaseAgentRunHooks``（``FRAMEQ_DRAFT_PLANNING=0``）。

    按 design D3 采用「安全做法」——转发**全部**六个事件（非只转 ``on_llm_request``），
    避免日后 inner hook 用到别的事件（如 ``on_tool_end``）时复合层静默漏转。每层调用都
    包 try/except，单层异常落 ``[draft]`` 日志、不中断主循环；``on_before_complete``
    的两层投票合并为 AND（任一否决即否决），抛异常的层按 ``BaseAgentRunHooks`` 默认放行，
    以免异常把循环卡死。
    """

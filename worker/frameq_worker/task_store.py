...
def write_task_manifest(
    ...
    *,
    ...
    ``draft_seed_insight_id`` preserve/clear rule (D7/D8):
    ...
    - ``None``: write ``null`` (clear the seed). Used by ``target="insights"``
      regen — insights changed, the old seed id is now invalid (D8).
    - ``int``: write that id. Used by ``target="draft"`` generation
      (``request.insight_id``).

    ``draft_path`` / ``has_draft`` always reflect the current state of
    ``ai/draft.md`` on disk (D7: artifacts are independent — insights regen does
